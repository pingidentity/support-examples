...
<%	
	// initialize the showAdvanced flag
	...
	String hostPF = spConfigProperties.getProperty(SampleConstants.SAMPLE_PF_HOST);
	String accountLinking = spConfigProperties.getProperty(SampleConstants.SAMPLE_ACCOUNT_LINKING);
	String useSSL = spConfigProperties.getProperty(SampleConstants.SAMPLE_USE_SSL);
	String attributeNamesList = spConfigProperties.getProperty(SampleConstants.SAMPLE_ATTRS_NAMES_LIST);		
	String httpPort = spConfigProperties.getProperty(SampleConstants.SAMPLE_PF_HTTP_PORT);		
	String httpsPort = spConfigProperties.getProperty(SampleConstants.SAMPLE_PF_HTTPS_PORT);		
	String wsUname = spConfigProperties.getProperty(SampleConstants.SAMPLE_PF_WS_UNAME);
	String wsPwd = spConfigProperties.getProperty(SampleConstants.SAMPLE_PF_WS_PWD);
...
<html>
	<head>
		...
		<title>SP Integration Configuration</title>
...
<body>
	...
	<div id="right_header" style="background-image: url(<%=request.getContextPath()%>/images/sp_head_02.gif)">	
		...
		<tr height="100">
		 ...
		 <tr>
		 	<td>
			<a href="<%= URLUtil.getServiceProviderHomeUrl(request) %>">Service Provider Home</a>
			</td>
		</tr>
		</table> 
	</div>
	...
	<div id="content">
   		<form name="config" action="<%= URLUtil.getConfigOptionsFormPostUrl(request, showAdvanced)%>" method="POST">
		<table class="cell" style="margin-left: auto; margin-right: auto;">
			<tr>
				<td colspan="6" style="text-align: center;">
					<h1>SP Application Configuration <br/> (<%= ConfigManager.getSpConfigFileName() %>)</h1>
					<hr class="cell"/>
				</td>
			</tr>
		...
		<FORM ENCTYPE='multipart/form-data' method='POST' action=''>            					   
			<table class="cell" style="margin-left: auto; margin-right: auto;">
				<tr>
					<td colspan="6" style="text-align: center;">
						<h1>SP OpenToken Configuration <br/> (<%= ConfigManager.getOpenTokenConfigFileName() %>)</h1>
						<hr class="cell"/>
					</td>
				</tr>
				<%
